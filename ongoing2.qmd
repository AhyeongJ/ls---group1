...
```{python}
pop_converted = pop.iloc[:18,1:14]
for col in pop_converted.columns:
    pop_converted[col] = pop_converted[col].str.replace(',', '').astype(int)
pop_converted
pop_converted.dtypes
```
...
```{python}
pop_total = pop_converted['총 인구수']
pop_under20 = pop_converted[['0~9세', '10~19세']].sum(axis = 1).transpose()
pop_2050 = pop_converted[['20~29세', '30~39세', '40~49세', '50~59세']] \
                        .sum(axis = 1).transpose()
pop_over60 = pop_converted[['60~69세', '70~79세', '80~89세', '90~99세', '100세 이상']] \
                         .sum(axis = 1).transpose()
pop_under20.index = ['전국', '서울', '부산', '대구', '인천', ]
pop_merged = pd.merge(pop_total, pop_under20, left_index=True, right_index=True, how='inner')
